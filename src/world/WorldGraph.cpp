...
void WorldGraph::forward_fill()
{
    while (hasNullNode() && !item_pool.empty()) 
    {
        ...
        // check all rooms that can be reached with the currently owned_items
        ...
        if (nullNodes.empty())
            break; // there are no reachable nodes that could get an item
        // pick a random node
...
void WorldGraph::log_debug() const
{
    for (const auto& [name, node] : nodes) 
    {
        ...
        if (node->isBoss)
            TraceLog(LOG_INFO, "This is the boss room");
...
bool WorldGraph::testReachability()
{
    ...
    if (numRNodes < nodes.size())
    {
        size_t diff = nodes.size() - numRNodes;
        TraceLog(LOG_WARNING, "There are %d unreachable nodes.", diff);
        return false;
    }
    else
    {
        TraceLog(LOG_INFO, "All nodes can be reached");
        return true;
    }
}
...
void WorldGraph::search()
{
    ...
    while (true) 
    {
        if (!start)
        {
            TraceLog(LOG_WARNING, "The start node has not been set.");
            return;
        }
...
std::unordered_set<std::shared_ptr<Node>> WorldGraph::getReachableNodes()
{
    std::unordered_set<std::shared_ptr<Node>> visited;
    if (!start)
        // the start Node hasn't been set yet
        return visited;

    std::deque<std::shared_ptr<Node>> checked;
    checked.push_back(start);
    visited.insert(start);

    while (!checked.empty())
    {
        const std::shared_ptr<Node>& node = checked.front();
        // check for each edge if the owned items let you traverse it 
        for (const auto& edge : node->edges)
        {
            ...
            {
                // you successfully went into the room that the edge points to
                // now that room can be checked in the next iteration
                checked.push_back(edge.target);
                visited.insert(edge.target);
            }
        }
        checked.pop_front();

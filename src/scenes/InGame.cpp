...
void InGame::processTileObject(const TileObject& obj, uint8_t currentState, std::unordered_map<uint32_t, ObjectState>& objectStates, const nlohmann::json& spriteData)
{
    if (!obj.visible) 
        return;
...
void InGame::loadTilemap() {
    ...
    for (const auto& obj : tileMap->getObjects()) {
        processTileObject(obj, currentState, objectStates, spriteData);
    }
